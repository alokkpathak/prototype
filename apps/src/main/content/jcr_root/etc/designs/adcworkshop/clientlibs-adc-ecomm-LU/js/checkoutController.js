$(document).ready(function() {
	...
	if ($('body').hasClass('Checkoutpage')) {
		...
		if (isCustURL != null) {
			app.appGateway.ajaxGet(isCustURL, checkoutCustSuccessCallback, checkoutCustErrorCallback);
		}
	}
    ...
    $('#Shipping_AddresssFind_overlay').click(
        function(event) {
            // to add code
            findSource = "overlay";
            postcode = $("#shipping_postCodePrefix").text()+$("#contact_postCode_overlay").val();
            expressCheckout.prototype.getQASOnDemandAddress(postcode, event);
    ...
    expressCheckout.prototype.init = function() {
        ...
        expressCheckout.prototype.getFinalPaymentDetails = function() {
            ...
            app.appGateway.ajaxGet(finalPaymentUrl, successfinalpaymentordersdata, errorfinalpaymentordersdata);
        }
        ...
        $(document).on("click", "#editAddr", function() {
            ...
            $.each(json.address, function(i, val) {
                if (this.entity_id == selected) {
                    ...
                    postcodeFormatted = selectedObj.postcode;
                    if($('#'+formName+'_postCodePrefix').text().length){
                   	 postcodeFormatted = (selectedObj.postcode).slice($('#'+formName+'_postCodePrefix').text().length);
                    ...
                    $("#" + formName + "_postCodeForm").val(postcodeFormatted);
    ...
    expressCheckout.prototype.productsSelected = function() {
        ...
        app.appGateway.ajaxGet(viewCart, viewCartSuccess, viewCartError);
    }
    ...
    expressCheckout.prototype.validateForm = function() {
        ...
        var numericFields = [$_cardNumber, $_ccv, $_day, $_month, $_year];
        var $_numResult = $();
        $.each(numericFields, function() {
            $_numResult = $_numResult.add(this);
        });

		$('#shipping_phoneForm,#shipping_phone, #billing_phone, #billing_phoneForm').focusout('input', function() {
            var input = $(this);
            var isAlphaNumeric = objValidator.validatePhoneNumber(input);
            objValidator.toggleErrorMsg(input, isAlphaNumeric, "phone_Invalid_LU");
        });
    ...
    expressCheckout.prototype.addressList = function() {
        ...
        if (listAddressUrl != null) {
            ...
            app.appGateway.ajaxGet(listAddressUrl, listaddrSuccessCallback, listaddrErrorCallback);
        }
    }
    ...
    expressCheckout.prototype.personalDetails = function() {
        ...
        app.appGateway.ajaxGet(profiledetailsServiceUrl, profiledetailsgetsuccessCallback, profiledetailsgeterrorCallback);
    }
    ...
    $(document).on('click', "#edit-submit", function() {
        ...
        if ($("#overlay_ManualAddressData").hasClass("hidden")) {
            ...
            if($('#contact_postCode_overlay').val().startsWith("L-"))
            	{
            	postalCode = $('#contact_postCode_overlay').val();
            	}
            else
            	{
            	 postalCode = $('#shipping_postCodePrefix').text() + $('#contact_postCode_overlay').val();
            	}
            city = $('#contact_city').val();
        } else {
            addressfield1 = $('#shipping_address_one').val();
            addressfield2 = $('#shipping_addrss_two').val();
            if($('#shipping_postCodeForm').val().startsWith("L-"))
        	{
        	postalCode = $('#shipping_postCodeForm').val();
        	}
        else
        	{
        	 postalCode = $('#shipping_postCodePrefix').text() + $('#shipping_postCodeForm').val();
        	}
    ...
    expressCheckout.prototype.getPaypalForm = function(token) {
        ...
        } else if(fetchedCountryCookie == "FR" || fetchedCountryCookie == "LU") {
            plainstyle = "?compressed=false&language=fr&style=plain";
        }else {
    ...
    expressCheckout.prototype.getShipmentTypeDetails = function() {
        ...
        if (getshippingchargesURL != null) {
            app.appGateway.ajaxGet(getshippingchargesURL, shipmethodSuccessCallback, shipmethodErrorCallback);
        }
    }

    function shipmethodSuccessCallback(data) {
		if (data.messages.allErrors == ""){
            if(data.shipping.list[0].config.express!=null){
			...
			if ($("#contact_title").val() == "" || $("#contact_title").val() == 'select') {
				...
				if (readCookie('username') != null && readCookie('username') != '' && loggedInCustomer) {
					app.appGateway.ajaxGet(profiledetailsServiceUrl, profileSuccessCallback, profileFailureCallback);
				}
			}
		} else {
			var errorCode = data.messages.error[0].code;
